    ...
    def handle(self, df, _n_neighbors=3):
        ...
        if self.missing_num or self.missing_categ:
            ...
            if self.count_missing != 0:
                logger.info('Found a total of {} missing value(s)',
                            self.count_missing)
                df = df.dropna(how='all')
                df.reset_index(drop=True)

                if self.missing_num:  # numeric data
                    logger.info('Started handling of NUMERICAL missing values... Method: "{}"', str(
                        self.missing_num).upper())
                    # automated handling
                    if self.missing_num == 'auto':
                        ...
                        df = MissingValues._impute(
                            self, df, imputer, type='num')
                ...
                if self.missing_categ:  # categorical data
                    logger.info('Started handling of CATEGORICAL missing values... Method: "{}"', str(
                        self.missing_categ).upper())
                    # automated handling
            ...
            logger.info(
                'Completed handling of missing values in {} seconds', round(end-start, 6))
        else:
                ...
                if feature in cols_num:
                    if df[feature].isna().sum().sum() != 0:
                        try:
                            ...
                            counter = df[feature].isna().sum(
                            ).sum() - df_imputed.isna().sum().sum()
                            ...
                            if (df[feature].fillna(-9999) % 1 == 0).all():
                                ...
                                df[feature] = df_imputed
                            if counter != 0:
                                logger.debug('{} imputation of {} value(s) succeeded for feature "{}"', str(
                                    self.missing_num).upper(), counter, feature)
                        except:
                            logger.warning('{} imputation failed for feature "{}"', str(
                                self.missing_num).upper(), feature)
        else:
            ...
            for feature in df.columns:
                if feature not in cols_num:
                    if df[feature].isna().sum() != 0:
                        try:
                            mapping = dict()
                            mappings = {k: i for i, k in enumerate(
                                df[feature].dropna().unique(), 0)}
                            mapping[feature] = mappings
                            df[feature] = df[feature].map(mapping[feature])

                            df_imputed = pd.DataFrame(imputer.fit_transform(
                                np.array(df[feature]).reshape(-1, 1)), columns=[feature])
                            counter = sum(1 for i, j in zip(
                                list(df_imputed[feature]), list(df[feature])) if i != j)
                            ...
                            mappings_inv = {v: k for k,
                                            v in mapping[feature].items()}
                            ...
                            logger.warning('{} imputation failed for feature "{}"', str(
                                self.missing_categ).upper(), feature)
        return df
    ...
    def _lin_regression_impute(self, df, model):
        ...
        for feature in cols_num:
            try:
                ...
                test_df = df[df[feature].isnull() == True].dropna(
                    subset=[x for x in df.columns if x != feature])
                ...
                train_df = df[df[feature].isnull() == False].dropna(
                    subset=[x for x in df.columns if x != feature])
                if len(test_df.index) != 0:
                    ...
                    try:
                        # might return error because logarithm of zero or negative values is undefined
...
# -9999 is a value that is highly unlikely to appear in the original data
                    # checks if the original target variable was an integer
                    if (df[feature].fillna(-9999) % 1 == 0).all():
                        # round back to INTs, if original data were INTs
                        ...
                        df[feature].update(test_df[feature])
                    logger.debug(
                        'LINREG imputation of {} value(s) succeeded for feature "{}"', len(pred), feature)
            except:
                logger.warning(
                    'LINREG imputation failed for feature "{}"', feature)
        for feature in df.columns:
            try:
                ...
                try:
                    ...
                    if len(test_df.index) != 0:
                        ...
                        if (df[feature].fillna(-9999) % 1 == 0).all():
                            ...
                            df[feature].update(test_df[feature])
                        logger.debug(
                            'LOGREG imputation of {} value(s) succeeded for feature "{}"', len(pred), feature)
                except:
                    logger.warning(
                        'LOGREG imputation failed for feature "{}"', feature)
        for feature in df.columns:
            try:
                # map categorical feature values back to original
                mappings_inv = {v: k for k, v in mapping[feature].items()}
...
class Outliers:
    ...
    def handle(self, df):
        ...
        if self.outliers:
            logger.info('Started handling of outliers... Method: "{}"',
            ...
            logger.info(
                'Completed handling of outliers in {} seconds', round(end-start, 6))
        else:
    ...
    def _winsorization(self, df):
        ...
        for feature in cols_num:
            ...
            lower_bound, upper_bound = Outliers._compute_bounds(
                self, df, feature)
            ...
            if counter != 0:
                logger.debug(
                    'Outlier imputation of {} value(s) succeeded for feature "{}"', counter, feature)
        return df
    ...
    def _delete(self, df):
        ...
        for feature in cols_num:
            ...
            if counter != 0:
                logger.debug(
                    'Deletion of {} outliers succeeded for feature "{}"', counter, feature)
        return df
...
class Adjust:
    ...
    def convert_datetime(self, df):
        ...
        if self.extract_datetime:
            logger.info(
                'Started conversion of DATETIME features... Granularity: {}', self.extract_datetime)
            start = timer()
            ...
            logger.info(
                'Completed conversion of DATETIME features in {} seconds', round(end-start, 4))
        else:
            ...
            for feature in cols_num:
                ...
                if (df[feature].fillna(-9999) % 1 == 0).all():
                    ...
                    try:
                        ...
                        counter += 1
                        logger.debug(
                            'Conversion to type FLOAT succeeded for feature "{}"', feature)
                    except:
                        logger.warning(
                            'Conversion to type FLOAT failed for feature "{}"', feature)
            end = timer()
            logger.info('Completed feature type conversion for {} feature(s) in {} seconds',
                        counter, round(end-start, 6))
        else:
...
class EncodeCateg:
    ...
    def handle(self, df):
        ...
        if self.encode_categ:
            ...
            logger.info('Started encoding categorical features... Method: "{}"', str(
                self.encode_categ[0]).upper())
            start = timer()
            for feature in target_cols:
                ...
                try:
                    ...
                    logger.debug(
                        'Skipped encoding for DATETIME feature "{}"', feature)
                except:
                    try:
                        if self.encode_categ[0] == 'auto':
                            ...
                            if df[feature].nunique() <= 10:
                                ...
                                logger.debug(
                                    'Encoding to ONEHOT succeeded for feature "{}"', feature)
                            # LABEL encode if not more than 20 unique values to encode
                                ...
                                logger.debug(
                                    'Encoding to LABEL succeeded for feature "{}"', feature)
                            ...
                            logger.debug('Encoding to {} succeeded for feature "{}"', str(
                                self.encode_categ[0]).upper(), feature)
                        elif self.encode_categ[0] == 'label':
                            df = EncodeCateg._to_label(df, feature)
                            logger.debug('Encoding to {} succeeded for feature "{}"', str(
                                self.encode_categ[0]).upper(), feature)
                    except:
                        logger.warning('Encoding to {} failed for feature "{}"', str(
                            self.encode_categ[0]).upper(), feature)
            end = timer()
            logger.info(
                'Completed encoding of categorical features in {} seconds', round(end-start, 6))
        else:
    ...
    def _to_onehot(self, df, feature, limit=10):
        ...
        if one_hot.shape[1] > limit:
            logger.warning(
                'ONEHOT encoding for feature "{}" creates {} new features. Consider LABEL encoding instead.', feature, one_hot.shape[1])
        # join the encoded df
        df = df.join(one_hot)
        return df
...
class Duplicates:
    ...
    def handle(self, df):
        if self.duplicates:
            logger.info('Started handling of duplicates... Method: "{}"', str(
                self.duplicates).upper())
            start = timer()
            original = df.shape
            try:
                ...
                if count != 0:
                    logger.debug(
                        'Deletion of {} duplicate(s) succeeded', count)
                else:
                    logger.debug('{} missing values found', count)
                end = timer()
                logger.info(
                    'Completed handling of duplicates in {} seconds', round(end-start, 6))
